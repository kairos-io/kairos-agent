...
var _ = Describe("Bundles and mounts hooks", func() {
	It("BundlePostInstall with bundles exercises the mount block", func() {
		c := makeConfig()
	...
	It("CustomMounts with mounts exercises the mount failure path", func() {
		c := makeConfig()
		c.Install = &sdkInstall.Install{
	...
	It("CopyLogs returns nil even when the mount fails", func() {
		c := makeConfig()
	...
	It("Finish chains all post-install hooks", func() {
		c := makeConfig()
		...
		Expect((Finish{}).Run(c, sp)).To(Succeed())
	})
})
