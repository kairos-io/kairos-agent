...
func (i *UpgradeAction) Run() (err error) {
	...
	i.cfg.UkiMaxEntries = 1
	if len(efiFiles) > i.cfg.UkiMaxEntries {
		...
		i.cfg.Logger.Infof("Removing: %s", filepath.Join(i.spec.EfiPartition.MountPoint, "loader", "entries", "kairos-"+versionList[0].Original()+".conf"))
		// Don't care about errors here, systemd-boot will ignore any configs if it cant find the efi file mentioned in it
		e := i.cfg.Fs.Remove(filepath.Join(i.spec.EfiPartition.MountPoint, "loader", "entries", "kairos-"+versionList[0].Original()+".conf"))
		if e != nil {
