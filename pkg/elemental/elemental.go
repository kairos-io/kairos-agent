...
func (e Elemental) MountPartition(part *v1.Partition, opts ...string) error {
	...
	if part.Path == "" {
		...
		device, err := utils.GetDeviceByLabel(e.config.Runner, e.config.Fs, part.FilesystemLabel, 10)
		if err != nil {
...
func (e *Elemental) CheckActiveDeployment(labels []string) bool {
	...
	for _, label := range labels {
		found, _ := utils.GetDeviceByLabel(e.config.Runner, e.config.Fs, label, 1)
		if found != "" {
